...
jest.mock('next/server', () => ({
  NextResponse: {
    json: (data: any, opts?: any) => ({ json: async () => data, status: opts?.status || 200, headers: opts?.headers || {} })
  }
}))
...
describe('POST rate limiting', () => {
  test('returns 429 and retry-after header after limit exceeded', async () => {
    const reqTemplate: any = { url: 'http://localhost/api/sample', json: async () => ({ label: 'A', value: 1 }), headers: { get: () => '127.0.0.1' } }
