...
function TestToastProvider({ children }: any) {
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { 'data-testid': 'toast-area' },
      toasts.map((x, i) => React.createElement('div', { key: i }, x.text)),
    ),
  );
}
...
function TestOptimistic() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    await globalMutate(
      ...
      (current: any[] = []) => [optimisticItem, ...current].slice(0, LIMIT),
      false,
    );
    try {
  ...
  return React.createElement(
    ...
    React.createElement(
      'ul',
      null,
      (data || []).map((d: any) => React.createElement('li', { key: d.id }, d.label)),
    ),
  );
}
