...
describe('server/db', () => {
  ...
  describe('Postgres mode', () => {
    ...
    it('getSamplePointsPaginated returns paginated rows', async () => {
      ...
      expect(mQuery).toHaveBeenCalledWith(
        'SELECT id, label, value FROM sample_data ORDER BY id LIMIT $1 OFFSET $2',
        [5, 0],
      );
    });
    ...
    it('getAllSamplePoints handles ECONNREFUSED error', async () => {
      ...
      await expect(db.getAllSamplePoints()).rejects.toThrow(
        'Database connection refused (ECONNREFUSED). Is Postgres running and is DATABASE_URL configured?',
      );
    });
    ...
    it('getSamplePointsPaginated handles other database errors and logs them', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[DB] getSamplePointsPaginated [postgres]:', {
        page: 1,
        limit: 10,
      });
  ...
  describe('In-memory mode', () => {
    ...
    it('getAllSamplePoints returns in-memory data', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[DB] getAllSamplePoints [memory]:', {
        count: points.length,
      });
    });
    ...
    it('getSamplePointsPaginated returns paginated in-memory data', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[DB] getSamplePointsPaginated [memory]:', {
        page: 1,
        limit: 3,
        count: points.length,
      });
